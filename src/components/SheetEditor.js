...
<style>
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  #chartOverlay, #tabMenuOverlay, #noteOverlay, #formatOverlay, #syncOverlay {
    position: fixed; inset: 0; background: rgba(10,30,5,.55); z-index: 9000;
    display: none; align-items: center; justify-content: center; padding: 12px;
  }
  #chartOverlay.show, #tabMenuOverlay.show, #noteOverlay.show, #formatOverlay.show, #syncOverlay.show { display: flex; }
  #chartModal, #tabMenuModal, #noteModal, #formatModal, #syncModal { background: #fff; border-radius: 14px; width: 100%; max-width: 340px; max-height: 90vh; overflow-y: auto; padding: 14px; }
  #chartModal h3, #tabMenuModal h3, #noteModal h3, #formatModal h3, #syncModal h3 { margin: 0 0 10px; font-size: 15px; color: #245f09; }
  #chartModal select, #chartModal input, #tabMenuModal input, #noteModal textarea, #formatModal select, #formatModal input, #syncModal input { width: 100%; padding: 8px; margin-bottom: 8px; border: 1px solid #cce9ae; border-radius: 6px; font-size: 13px; font-family: inherit; }
...
<div id="syncOverlay">
  <div id="syncModal">
    ...
    <span class="field-label">Código de la matriz (compartilo para colaborar)</span>
    <input id="syncDocId" type="text" placeholder="ej: MATRIZ-KCN-001" />
    <div class="row" style="justify-content:space-between;">
      <button class="cancel" id="syncCancel">Cerrar</button>
      <button class="ok" id="syncConnect">Conectar y sincronizar</button>
    </div>
  </div>
</div>
...
var syncEnabled = false, syncDocId = null, firebaseApp = null, dbRef = null, applyingRemote = false;
...
// ══════════════════════════════════════════════
var FIREBASE_CONFIG = {
  apiKey: "AIzaSyDUMMY-REEMPLAZAR-CON-TU-API-KEY",
  authDomain: "wc-corporate-editor.firebaseapp.com",
  databaseURL: "https://wc-corporate-editor-default-rtdb.firebaseio.com",
  projectId: "wc-corporate-editor"
};
...
document.getElementById('btnSync').addEventListener('click', function () {
  document.getElementById('syncDocId').value = syncDocId || '';
  document.getElementById('syncOverlay').classList.add('show');
});
document.getElementById('syncCancel').addEventListener('click', function () { document.getElementById('syncOverlay').classList.remove('show'); });
document.getElementById('syncConnect').addEventListener('click', function () {
  var id = document.getElementById('syncDocId').value.trim();
  if (!id) { alert('Ingresá un código para la matriz.'); return; }
  connectSync(id);
...
function connectSync(id) {
  try {
    ...
    syncDocId = id;
    syncEnabled = true;
    dbRef = firebase.database().ref('matrices/' + id);

    dbRef.on('value', function (snapshot) {
      var remote = snapshot.val();
      if (!remote) { pushFullState(); return; }
      applyingRemote = true;
      sheets = remote.sheets || sheets;
      sheetData = remote.sheetData || sheetData;
      sheetFormats = remote.sheetFormats || sheetFormats;
      sheetNotes = remote.sheetNotes || sheetNotes;
      colWidths = remote.colWidths || colWidths;
      if (sheets.indexOf(activeSheet) < 0) activeSheet = sheets[0];
      buildTable();
    ...
    setSyncStatus('error', 'Firebase no disponible');
  }
}

function pushFullState() {
  if (!syncEnabled || !dbRef || applyingRemote) return;
  dbRef.set({
    sheets: sheets, sheetData: sheetData, sheetFormats: sheetFormats,
    sheetNotes: sheetNotes, colWidths: colWidths, updatedAt: Date.now()
  }).then(function () { setSyncStatus('online', 'Sincronizado'); })
    .catch(function () { setSyncStatus('error', 'Error al guardar'); });
}
...
function autoSave() {
  try {
    ...
    if (syncDocId) localStorage.setItem('mcr_syncid_v6', syncDocId);
    if (!syncEnabled) document.getElementById('saveStatus').textContent = '💾 ' + new Date().toLocaleTimeString('es-CR');
    if (syncEnabled) pushFullState();
  } catch (e) {}
}
function restore() {
  try {
    ...
    var sid = localStorage.getItem('mcr_syncid_v6');
    ...
    if (sid) syncDocId = sid;
  } catch (e) {}
}

restore();
buildTable();
if (syncDocId) connectSync(syncDocId);
